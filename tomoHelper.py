...
class tomoHelper:
	...
	def getCost(self, measProbs, pureProbs, whichCost = "KL2"):
		cost = 0;
		if whichCost == "KL2":
			for i in range(measProbs.shape[0]):
				cost = cost + sum(pureProbs[i]*(np.log(pureProbs[i]) - np.log(measProbs[i])));
		return cost;
	
	def getWState(self, nVisible):
		W = np.zeros(2**nVisible);
		for i in range(nVisible):
			W[2**i] = 1;
		W = W/np.sqrt(nVisible);
		return W;
